...
function buildBookingConfirmationHtml(data: BookingEmailData) {
  const passengersRows = data.passengers
    .map(
      (p) =>
        `<tr>
...
<body>
  <div class="container">
    <div class="card">
      ...
      <div class="title">Your booking is confirmed, ${escapeHtml(data.leadName)}!</div>
      <div class="subtitle">We've confirmed your tour and processed your booking. Here are your details and payment information.</div>
      ...
      <div class="note">
        <strong>What happens next?</strong><br />
        Your booking is now confirmed. Please present this email at the tour starting point. For any changes or cancellations, contact us at least 24 hours before the tour.
      </div>
...
function buildBookingConfirmationText(data: BookingEmailData): string {
  return `Lanka Lagoon Tours - Booking Confirmed
...
Payment Link: https://pay.sample-lankalagoontours.lk/booking/${data.bookingNumber}

Please complete your payment and present this email at the tour starting point.`;
}
...
export async function sendBookingConfirmationEmail(data: BookingEmailData) {
  ...
  const subject = `Booking Confirmed: ${data.tourTitle} — ${data.bookingNumber}`;
  ...
  await transporter.sendMail({
    ...
    text: buildBookingConfirmationText(data),
  });
}
...
function buildContactCustomerHtml(data: ContactEmailData) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
<body>
  <div class="container">
    <div class="card">
      ...
      <div class="title">Thank you, ${escapeHtml(data.name)}!</div>
      <div class="subtitle">We've received your message and will get back to you shortly.</div>
      <div class="section-title">Your Message</div>
      <div class="note">${escapeHtml(data.message)}</div>
...
function buildContactAdminHtml(data: ContactEmailData) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    ...
    .label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: #6b7280; }
    .value { margin-top: 6px; font-size: 15px; font-weight: 600; color: #0f2e2c; }
    .message { margin-top: 12px; padding: 14px; border-radius: 12px; background: #f9fafb; border: 1px solid #e5e7eb; font-size: 14px; line-height: 1.6; color: #0f2e2c; white-space: pre-wrap; }
    .footer { margin-top: 24px; font-size: 12px; color: #9ca3af; text-align: center; }
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      <div class="brand">Lanka <span>Lagoon</span> Tours</div>
      <div class="eyebrow">New Contact Request</div>
      <div class="title">You have a new message from ${escapeHtml(data.name)}</div>
      <div class="subtitle">A visitor has reached out via the contact form.</div>
...
function buildContactCustomerText(data: ContactEmailData): string {
  return `Lanka Lagoon Tours - Message Received
...
Questions? Contact us at info@lankalagoontours.lk or call +94 76 344 3826.`;
}

function buildContactAdminText(data: ContactEmailData): string {
  return `Lanka Lagoon Tours - New Contact Request
...
export async function sendContactCustomerEmail(data: ContactEmailData) {
  const env = getEnv();
  const transporter = getTransporter();

  const subject = `Lanka Lagoon Tours — Message Received`;

  await transporter.sendMail({
    from: env.emailFrom,
    to: data.email,
    subject,
    html: buildContactCustomerHtml(data),
    text: buildContactCustomerText(data),
  });
}
