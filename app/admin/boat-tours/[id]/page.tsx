...
import { useBoatTourForm, normalizeBoatTour, createEmptyBoatTour, slugify } from '@/hooks/admin/useBoatTourForm'
...
export default function AdminBoatTours() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setTour(normalizeBoatTour(data, createEmptyBoatTour('')));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await updateBoatTourDetails(params.id as string, buildPayload());
    ...
    return (
        <AdminLayout>
            <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl">
                ...
                <Section title="Status" hint="Active tours are shown on the website, while inactive tours are hidden.">
                    <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm text-[#16302E]">Active</span>
                        </label>
                        <label className="flex items-center gap-2">
                            <input
                                type="radio"
                                name="status"
                                value="INACTIVE"
                                checked={tour.status === 'INACTIVE'}
                                onChange={() => update('status', 'INACTIVE')}
                            />
                            <span className="text-sm text-[#16302E]">Inactive</span>
                        </label>
                    </div>
                </Section>
